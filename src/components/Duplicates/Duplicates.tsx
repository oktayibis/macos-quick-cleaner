...
export function Duplicates() {
  const { duplicates, isLoadingDuplicates, scanDuplicates, deleteDuplicate } = useAppStore();
  ...
  // Get all duplicate files (excluding the first one in each group - the one to keep)
  ...
  const handleDeleteAll = () => {
    if (allDuplicateFiles.length === 0) return;
  ...
  const executeDelete = async () => {
    ...
    if (confirmDialog.type === 'single' && confirmDialog.path) {
      ...
      await deleteDuplicate(confirmDialog.path);
      ...
      for (let i = 0; i < paths.length; i++) {
        ...
        await deleteDuplicate(paths[i]);
      }
      setSelectedPaths(new Set());
    } else if (confirmDialog.type === 'all') {
      setProgress({ current: 0, total: allDuplicateFiles.length, item: '' });
      for (let i = 0; i < allDuplicateFiles.length; i++) {
        const file = duplicates.flatMap(g => g.files).find(f => f.path === allDuplicateFiles[i]);
        setProgress({ current: i, total: allDuplicateFiles.length, item: file?.name || allDuplicateFiles[i] });
        await deleteDuplicate(allDuplicateFiles[i]);
      }
      setSelectedPaths(new Set());
    }
  ...
  const getDialogProps = () => {
    if (confirmDialog.type === 'single' && confirmDialog.path) {
      ...
      return {
        ...
        message: `Are you sure you want to delete "${file?.name}"? The original will be kept.`,
        itemCount: 1,
        totalSize: formatBytes(group?.file_size || 0),
      };
    } else if (confirmDialog.type === 'selected') {
      return {
        title: 'Delete Selected Duplicates?',
        message: 'Are you sure you want to delete all selected duplicate files? Original files will be kept.',
      ...
      return {
        ...
        message: 'Are you sure you want to delete ALL duplicate files? This will keep one copy of each file.',
        itemCount: allDuplicateFiles.length,
        totalSize: formatBytes(totalWasted),
      };
    }
  };
  ...
  return (
    <div className="main-content">
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', alignItems: 'center', flexWrap: 'wrap' }}>
        <button
          className={`btn btn-primary ${isLoadingDuplicates ? 'loading' : ''}`}
          onClick={() => scanDuplicates(minSizeMb)}
          disabled={isLoadingDuplicates || isDeleting}
        >
        ...
        {allDuplicateFiles.length > 0 && (
          <>
            <button
              ...
              onClick={selectedPaths.size === allDuplicateFiles.length ? deselectAll : selectAllDuplicates}
              disabled={isDeleting}
            >
              {selectedPaths.size === allDuplicateFiles.length ? (
                <><Square size={18} /> Deselect All</>
              ) : (
                <><CheckSquare size={18} /> Select All Duplicates</>
              )}
            </button>
            ...
            >
              <Trash2 size={18} />
              Delete All Duplicates ({formatBytes(totalWasted)})
            </button>
          </>
        )}
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="card-title">Duplicate Groups ({duplicates.length})</h3>
        </div>

        {duplicates.length === 0 ? (
          <div className="empty-state">
            <Copy size={48} className="empty-state-icon" />
            <p className="empty-state-title">No duplicates scanned yet</p>
                ...
                >
                  ...
                  <div className="list-item-content">
                    ...
                    <div className="list-item-subtitle">
                      {formatBytes(group.file_size)} each • {formatBytes(group.total_wasted)} wasted
                    </div>
                  </div>
                  ...
                  <div style={{ padding: '0 16px 16px 16px' }}>
                    {group.files.map((file, index) => (
                      ...
                      >
                        ...
                        <File size={16} style={{ flexShrink: 0, opacity: 0.5 }} />
                        ...
                        {index > 0 ? (
                          ...
                          <span className="badge badge-image" style={{ fontSize: '0.625rem' }}>Keep Original</span>
                        )}
                      </div>
                    ))}
                  </div>
